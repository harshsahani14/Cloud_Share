  ...
  return (
    <div className=' py-20 bg-gray-50 '>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className=" mt-16 space-y-12 lg:space-y-0 lg:grid-cols-3 lg:gap-8 lg:grid">
            {
              pricingPlans.map((plan, index) => (
                <div key={index} className={` flex flex-col rounded-lg shadow-lg overflow-hidden ${plan.highlighted ? 'border-2 border-purple-500 transform scale-105' : 'border border-gray-200' } `}>
                  <div className={ ` px-6 py-8 bg-white
                    ${plan.highlighted ? 'bg-gradient-to-br from-purple-50  to-white' : '' }`}>
                      <div className="flex justify-between items-center">
                        ...
                        { plan.highlighted && ( 
                          ...
                          <span className="inline-flex items-center px-3 py-0.5 text-sm rounded-full font-medium bg-purple-100 text-purple-800">
                            Popular
                          </span>
                        )}
                      </div>
                   ...
                   <div className="flex-1 flex-col justify-between px-6 pt-6 pb-6 bg-gray-50 space-y-6">
                    <ul className="space-y-4">
                      { plan.features.map((feature, fIndex) => (
                      <li key={fIndex} className="flex items-start">
                        <div className="flex-shrink-0">
                              <Check className={ ` h-5 w-5 text-purple-500 ${feature ? '' : 'invisible'}`} />
                          </div> 
